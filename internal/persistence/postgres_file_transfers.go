...
// ListFileTransfers returns one actor's transfer history without ever loading
// another actor's rows into the application. File-transfer IDs are generated by
// idgen.New and are sortable by creation time, so ID DESC provides a stable
// newest-first order even while a pending transfer has no started_at value.
func (s *PostgresStore) ListFileTransfers(ctx context.Context, actorID, status string, limit, offset int) ([]FileTransfer, int, error) {
	...
	if limit <= 0 {
		limit = FileTransferListDefaultLimit
	}
	if limit > FileTransferListMaxLimit {
		limit = FileTransferListMaxLimit
	}
	if offset < 0 {
		offset = 0
	}
	if offset > FileTransferListMaxOffset {
		offset = FileTransferListMaxOffset
	}

	whereClause := `actor_id = $1`
	filterArgs := []any{actorID}
	if status != "" {
		filterArgs = append(filterArgs, status)
		whereClause += fmt.Sprintf(` AND status = $%d`, len(filterArgs))
	}

	var total int
	if err := s.pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM file_transfers WHERE `+whereClause,
		filterArgs...,
	).Scan(&total); err != nil {
		return nil, 0, err
	}
	...
	rows, err := s.pool.Query(ctx,
		fmt.Sprintf(
			`SELECT %s FROM file_transfers WHERE %s ORDER BY id DESC LIMIT $%d OFFSET $%d`,
			fileTransferColumns,
			whereClause,
			limitPosition,
			offsetPosition,
	...
	out := make([]FileTransfer, 0, min(limit, total))
	for rows.Next() {
