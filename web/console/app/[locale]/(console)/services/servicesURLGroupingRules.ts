export function parseNeverGroupRuleLine(line: string): [string, string] | null {
  const separators = ["<=>", "=>", ","];
...
export function groupingPairKey(left: string, right: string): string | null {
  const a = left.trim();
  const b = right.trim();
  if (!a || !b || a === b) {
    return null;
...
export function appendNeverGroupRule(existingRaw: string, leftURL: string, rightURL: string): string {
  const leftIdentity = canonicalURLIdentity(leftURL);
  const rightIdentity = canonicalURLIdentity(rightURL);
  if (!leftIdentity || !rightIdentity) {
    throw new Error("Both URLs must be valid to remove an alternative URL.");
  }
  const incomingKey = groupingPairKey(leftIdentity, rightIdentity);
  if (!incomingKey) {
    throw new Error("Cannot create a never-group rule for identical URLs.");
  }
  ...
  for (const line of lines) {
    const parsedRule = parseNeverGroupRuleLine(line);
    if (!parsedRule) {
    ...
    const key = groupingPairKey(parsedLeft, parsedRight);
    if (!key || seen.has(key)) {
      continue;
    }
    seen.add(key);
