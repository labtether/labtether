...
describe("RemoteViewTabBar accessibility", () => {
  it("renders separate, named tab and close buttons", async () => {
    ...
    await act(async () => {
      root.render(
        <RemoteViewTabBar
          tabs={[{ id: "new", type: "new", label: "New Tab", connectionState: "idle" }]}
          activeTabId="new"
          onAddTab={vi.fn()}
    ...
    expect(container.querySelector('[role="tablist"]')?.getAttribute("aria-label")).toBe("Remote view tabs");
