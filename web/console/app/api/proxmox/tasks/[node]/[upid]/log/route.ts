...
export async function GET(request: Request, { params }: Params) {
  ...
  try {
    const response = await fetch(
      `${base.api}/proxmox/tasks/${encodeURIComponent(node)}/${encodeURIComponent(upid)}/log${querySuffix}`,
      { cache: "no-store", headers: authHeaders, signal: AbortSignal.timeout(10_000) },
    );
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
      return NextResponse.json(payload ?? { error: "failed to fetch task log" }, { status: response.status });
    }
    return NextResponse.json(payload ?? {});
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "failed to fetch task log" },
      { status: 502 },
    );
  }
}
