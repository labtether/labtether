...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    const response = await fetch(
      `${base.api}/proxmox/tasks/${encodeURIComponent(node)}/${encodeURIComponent(upid)}/stop${querySuffix}`,
      { method: "POST", cache: "no-store", headers: authHeaders, signal: AbortSignal.timeout(10_000) },
    );
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
      return NextResponse.json(payload ?? { error: "failed to stop task" }, { status: response.status });
    }
