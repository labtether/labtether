...
export async function GET(request: Request, { params }: Params) {
  ...
  try {
    const response = await fetch(
      `${base.api}/proxmox/nodes/${encodeURIComponent(node)}/updates${querySuffix}`,
      { cache: "no-store", headers: authHeaders, signal: AbortSignal.timeout(15_000) },
    );
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
      return NextResponse.json(payload ?? { error: "failed to fetch updates" }, { status: response.status });
    }
