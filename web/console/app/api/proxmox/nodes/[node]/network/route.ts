...
export async function GET(request: Request, { params }: Params) {
  ...
  try {
    const response = await fetch(
      `${base.api}/proxmox/nodes/${encodeURIComponent(node)}/network${querySuffix}`,
      { cache: "no-store", headers: authHeaders, signal: AbortSignal.timeout(10_000) },
    );
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
      return NextResponse.json(payload ?? { error: "failed to fetch node network" }, { status: response.status });
    }
    return NextResponse.json(payload ?? {});
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "failed to fetch node network" },
      { status: 502 },
    );
  }
}
