...
export async function POST(request: Request) {
  ...
  const secretCandidates = [typeof parsed.token === "string" ? parsed.token : ""];

  try {
    const response = await fetch(`${base.api}/connectors/homeassistant/test`, {
      method: "POST",
      cache: "no-store",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        sanitizeTestRouteError(payload, "failed to test home assistant connection", secretCandidates),
    ...
    return NextResponse.json(
      sanitizeTestRouteError(error instanceof Error ? error.message : "", "failed to test home assistant connection", secretCandidates),
      { status: 502 },
    );
  }
}
