...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(payload ?? { error: "failed to load notification channels" }, { status: response.status });
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await safeJSON(response);
    if (!response.ok) {
      return NextResponse.json(payload ?? { error: "failed to create notification channel" }, { status: response.status });
    }
    return NextResponse.json(payload ?? {});
  } catch (error) {
    return NextResponse.json(
